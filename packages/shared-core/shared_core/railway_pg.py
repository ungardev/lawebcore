"""Railway PostgreSQL asyncpg client — the actual DB connection layer.

This module is the real database access layer. The old "supabase_rest" name
was a misnomer: it uses asyncpg directly against Railway PostgreSQL,
not the Supabase REST API or Supabase Auth.
"""
...
class RailwayPg:
    ...
    async def _ensure_pool(self) -> asyncpg.Pool:
        if self._pool is None:
            logger.info("[railway_pg] Creating new asyncpg pool for %s", self._dsn.split("@")[-1])
            ...
            logger.info("[railway_pg] Pool created successfully")
        return self._pool

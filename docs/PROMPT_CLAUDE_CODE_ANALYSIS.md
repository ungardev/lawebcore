...
> **Hito actual:** Hito 36 ✅ · Fable 5 P0s ✅ · B1/B2/B3 ✅ · credentials ✅ · N-1..N-4 OpenAPI ✅ · B-FE-7/15 paridad ✅ · **D-1..D-7/R-1/R-2 DeepSeek+RAG coherente ✅** · **E2E ❌ PENDIENTE (UI o script)**
> **DeepSeek:** `deepseek-v4-flash` · thinking OFF + cache ON (testeado) · smoke: `python -m scripts.test_deepseek` · ⚠️ `ENABLE_AI_ANALYZER` OFF por default (Railway decide)
> **E2E Test:** ❌ FALLÓ run `10a59ecf` — 188 handles → 0 candidatos · Todos los fixes aplicados (backend N + frontend B-FE + LLM/RAG D/R) · E2E pendiente
> **NUEVO (07-sep, Run 3):** `docs/FIXES_RUN3_DELIVERY_KEYWORDS_07-09-26.md` — 3 candidatos calificados pero INSERT falló (discovery_query sin migrar) → F1 migración auto-aplicada · F2 extractor medias xdt + script diagnóstico · F3 NULL≠0 en candidatos · F4 mensaje verdadero · F5 keywords v2 de identidad de creador + geo condicional + exclusión brand-own. 242 passed
...
| **37** | *`docs/FIXES_FRONTEND_STATUS_PARITY_BFE7_04-09-26.md`* | **2026-09-04** | **GLM 5.3 Flash** | **B-FE-7/B-FE-15 FIXED — la UI ya puede mostrar candidatos:** trazado del flujo UI reveló que useRunPolling solo detenía polling en estados legacy (`completed/partial/failed/cancelled`) que el backend jamás emite (emite `delivered/degraded/empty/inconsistent/aborted_budget/failed`) → un run exitoso giraba infinito y los candidatos nunca se fetcheaban (ni recargando página — loadConversation con el mismo filtro) · `TERMINAL_RUN_STATUSES`/`CANDIDATE_RUN_STATUSES` como constantes exportadas + `runStatus`/`runError` expuestos · auto-reload del chat cuando polling termina sin candidatos (muestra la causa honesta del worker) · wizard de un click (`startWizardSearch`: crea conversación + auto-confirma + polling + reload) · TikTok/YouTube deshabilitados "Próximamente" (pipeline IG-only) · 3 tests de contrato FE↔BE que parsean el TS y lo comparan contra `RunStatus` — drift futuro rompe CI · 197 passed, tsc + eslint limpios |
| **38** | *`docs/FIXES_DEEPSEEK_RAG_04-09-26.md`* | **2026-09-04** | **GLM 5.3 Flash** | **Auditoría DeepSeek-V4-Flash + RAG — conectado SÍ, coherente PARCIAL; fixed:** D-1 `brief_parser` max_tokens=300 truncaba el JSON de ~25 campos → brief degradado a heurístico EN SILENCIO (1200/1500 + truncation-retry) · D-2 `ENABLE_AI_ANALYZER=False` default = STEP 5 IA era no-op silencioso (warning por run + documentado; decisión Railway) · D-3 cliente ChatOpenAI reconstruido por llamada + to_thread → cacheado + `ainvoke` · D-4 retries sobre 401/400 → fail-fast solo-retry 429/5xx · D-5 truncation `finish_reason=length` → retry max_tokens×2 (adiós hack `+'"}'`) · D-6 prompt analyzer pedía array pero json_object exige objeto → `{"scores":[...]}` · D-7 `llm.py::get_llm` dead code SIN thinking → eliminado · R-1 `rag_engine` tiraba el historial de conversación a la basura (RAG mono-turno) → nuevo `history=` en `complete()` · R-2 embeddings fallback devolvía vectores cero a document_chunks → None + skip · Smoke `scripts/test_deepseek.py` (~$0.001) + 10 tests · 207 passed, 0 regresiones |
| **39** | *`docs/FIXES_WIZARD_COHERENCIA_HIKERAPI_04-09-26.md`* | **2026-09-04** | **GLM 5.3 Flash** | **Wizard coherente con HikerAPI:** el paso Nicho decía "detectados automáticamente" (falso — presets hardcodeados) y sus presets quemaban llamadas (`'pet care'` en inglés → cuentas gringas muertas por geo filter; `'perrosvzla'` como keyword en vez de hashtag) · Presets rediseñados 12/12 industrias en español con intención de búsqueda de cuentas VE (`veterinaria`, `adiestramiento canino`...) · ❌ en vez de ✅ para quitar · HashtagSuggestions reescrito alineado a `VE_NICHE_HASHTAGS` del backend (12 industrias, "las ejecuta el pipeline") · Nota de ejecución real en paso 4 (top-6/recent-4) · Paso 6 con **Plan de búsqueda real** (keywords×3 variantes geo, hashtags ejecutados, ~25 enriquecidos) · tsc + eslint limpios |
| **40** | *`docs/FIXES_BRIEF_FIELDS_LEAK_04-09-26.md`* | **2026-09-04** | **GLM 5.3 Flash** | **Fuga de brief cerrada + verdad en wizard:** verificado que los nichos NO viajan directos a HikerAPI (DeepSeek genera keywords DESDE los nichos vía fingerprint; solo los primeros 6 keywords se ejecutan) → el contador "≈21 búsquedas" era falso · **DECISIÓN: NO reordenar nichos/keywords hasta el primer E2E** (optimizar sin datos viola disciplina Fable 5.1) · **FIX fuga real:** `DiscoverySearchRequest` no tenía `additional_context`/`competitor_brands`/`influencer_preferences`/campos de campaña → el "NO tiendas" del wizard y los tiers de PDFs se evaporaban al crear el run (100% perdidos) · +12 campos opcionales al schema (backward compat) + pasados en discovery.py + 3 tests de roundtrip · Wizard ahora dice la verdad (nichos = contexto IA, 6 keywords IA ×3 variantes = 18 búsquedas) · 210 passed |
| **41** | *`docs/FIXES_RAILWAY_WORKER_RESILIENCE_05-09-26.md`* | **2026-09-05** | **GLM 5.3 Flash** | **Resiliencia Railway (detectado por Ungar en deploy logs ANTES del E2E):** 🔴 **worker arq MUERTO desde 23:17** — blip de Redis (Connection reset by peer) mató el proceso daemon; la API seguía en /health 200 → Railway nunca reinicia → discovery runs enqueued para siempre · FIX: `arq_worker_supervised()` con backoff 5s→60s · 🟡 cron 09:00 diario fallaba (`is_active=eq.true` como STRING → DataError asyncpg) → coerción PostgREST true/false→bool en `_parse_filters` + cron blindado · **BONUS bug latente:** `col=in.(...)` con "=" se trataba como igualdad contra "('a','b')" → 0 filas SIEMPRE → el modo Analizar habría devuelto 0 perfiles; ahora se expande a IN con coerción · 13 tests nuevos, 223 passed |
| **42** | *`docs/FIXES_RAILWAY_WORKER_RESILIENCE_05-09-26.md` (corrección v2)* | **2026-09-07** | **GLM 5.3 Flash** | **Corrección v2 del supervisor — bug de la v1 detectado por Ungar en deploy:** `run_worker` de arq es SYNC (maneja su propio loop) — la v1 lo await'eo dentro de `asyncio.run` → "RuntimeError: This event loop is already running" → **worker muerto AL ARRANCAR** en Railway (sin "Starting worker" en logs) · FIX v2: supervisor sync puro + event loop nuevo por intento + `time.sleep` backoff (correcto: el proceso existe solo para el worker) · **Lección registrada:** el fake del test debe replicar el contrato REAL de la librería — la v1 validó un fake async que no existía · Tests v2 al contrato real (fake sync): 14/14, 224 passed |
| **43** | *`docs/FIXES_SCORING_CRASH_E2E_07-09-26.md`* | **2026-09-07** | **GLM 5.3 Flash** | **PRIMER E2E REAL vía UI — pipeline llegó a scoring y murió por 1 bug (ya fixed):** TODO lo demás funcionó en producción: worker vivo (v2) · wizard un-click · N-1 pre-flight `/sys/balance` 200 · DeepSeek profile con elite_data PERSISTIDA (B-NEW-2 = no-issue en prod) · 168 handles descubiertos · 25 enriquecidos · **N-3 medias fetch funcionando** (ER real) · progreso en chat · **CRASH:** `drop_profile` con `detail={'reason':...}` colisionaba con `reason=reason.value` → TypeError → run muerto tras gastar ~$1.4 · **8 call sites** con el patrón (4 geo + prefilter + rerank + bots + political) · FIX: guard central en `drop_profile` (renombra colisiones a `detail_*`) + claves `reason`→`why` en los 8 sites + dedupe de variante geo duplicada ("dog chow venezuela venezuela") · 4 tests nuevos, 228 passed |
| **44** | *`docs/FIXES_RUN3_DELIVERY_KEYWORDS_07-09-26.md`* | **2026-09-07** | **GLM 5.3 Flash** | **RUN 3 (75855ad1, $1.56) — llegó al ÚLTIMO paso: 3 candidatos calificados con brand_fit IA, INSERT falló:** `discovery_candidates.discovery_query` NO existe en prod (migración 111 jamás aplicada) → batch + 3 fallbacks fallidos → 0 entregados · **F1 FIXED:** columnas `discovery_query`/`brand_fit` en la migración auto-aplicada del startup (idempotente) · **F2:** extractor medias v2 (forma xdt moderna) + log diagnóstico + script WSL `test_user_medias.py` (ER real: medias fetch OK pero posts_analyzed=0 — forma no reconocida) · **F3 NULL≠0:** ER/following/posts_count/expected_engagement persistidos sin zeros falsos (camelCase zeros tapaban valores enriquecidos) · **F4:** mensaje verdadero si el insert falla (antes: "3 calificados" + "ninguno califica" en contradicción) · **F5 keywords inteligentes:** prompt v2 = IDENTIDAD DE CREADOR (no buy-intent, sin geo embebida, additional_context del cliente en el prompt) + geo condicional en worker + exclusión brand-own (`_brand_tokens` bigrams — @dogchowve ya no se entrega como creador, sin falso positivo "chow") · 242 passed |

...
> **NUEVO (07-sep):** Corrección v2 del supervisor arq — la v1 await'eo run_worker (sync) dentro de asyncio.run → worker muerto al arrancar. v2: supervisor sync puro + loop nuevo por intento + tests al contrato REAL (fake sync). Ver `docs/FIXES_RAILWAY_WORKER_RESILIENCE_05-09-26.md` §Corrección v2

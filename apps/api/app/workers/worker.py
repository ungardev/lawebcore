...
async def discovery_run_task(ctx, run_id: str) -> dict:
    ...
    try:
        ...
        # HITO 24 — MODO EXPLORAR / ANALIZAR
        ...
        # G14 — MODO ANALIZAR: cargar candidatos del run padre (explore)
        # y enriquecer SOLO los handles_to_analyze. Sin esto, el analyze run
        # re-ejecutaría discovery y no encontraría los handles del padre.
        ...
        if is_analyze_mode and parent_run_id:
            ...
            for c in parent_candidates:
                ...
                profiles[handle] = {
                    "username": handle,
                    "full_name": c.get("full_name") or raw.get("full_name", ""),
                    "fullName": c.get("full_name") or raw.get("fullName", ""),
                    "bio": c.get("bio") or raw.get("bio", ""),
                    "biography": c.get("bio") or raw.get("biography", ""),
                    "avatar_url": c.get("avatar_url") or raw.get("avatar_url", ""),
                    "profilePicUrl": c.get("avatar_url") or raw.get("profilePicUrl", ""),
                    ...
                    "isBusinessAccount": raw.get("isBusinessAccount", False),
                    "is_verified": raw.get("is_verified", False) or raw.get("verified", False),
            ...
            logger.info(
                ...
                handles_loaded=len(profiles),
                requested=len(brief.handles_to_analyze),
            )
            await _save_progress_message(
                run_id,
                f"🔍 Analizando {len(profiles)} perfiles seleccionados del run anterior...",
            )
        ...
        if _skip_discovery:
            print("[discovery_run_task] ANALYZE MODE — skipping STEP 1/2/2.5/3 discovery", flush=True)
        ...
        for handle, p in profiles.items():
            ...
            if followers == 0:
                if is_explore_mode:
                    ...
                    #
                    # HITO 26 FIX: el dict DEBE tener la misma forma que el del
                    # camino normal. `upsert_many` deriva las columnas de
                    # records[0].keys(); claves como "profile" o "rough_score"
                    # no son columnas de discovery_candidates y hacían fallar
                    # el INSERT entero → 0 candidatos insertados SIEMPRE.
                    rough = rough_score_map.get(handle, 0.0)
                    if handle not in rough_score_map:
                        # Solo los prefiltrados (top-N por rough score) llegan aquí.
                        continue
                    ...
                    scored.append({
                        ...
                        "bio": bio_ex,
                        "avatar_url": (
                            p.get("profilePicUrlHD")
                            or p.get("profilePicUrl")
                            or p.get("avatar_url")
                            or (f"https://instagram.com/{handle}/profile_picture" if handle else "")
                        ),
                        ...
                        "roi_estimate": None,
                        "rationale": (
                            "Descubierto en modo Explorar (sin enriquecer). "
                            "Señal de nicho/geo derivada de la bio y el usuario. "
                            "Seleccionalo para analizarlo en profundidad."
                        ),
                        "tier": None,
                        "is_tienda": False,
                        "status": "new",
        ...
        if len(qualified) == 0:
            ...
            if is_explore_mode:
                ...
                top = qualified[0].get("match_score", 0) if qualified else 0
                await _save_progress_message(
                    run_id,
        ...
        await _run_update_metadata(run_id, {
            ...
            "is_analyze_mode": is_analyze_mode,
            "parent_run_id": parent_run_id,
        })
        ...
        # para reconciliación Redis↔DB.
        #
        # HITO 26 FIX: contabilidad auxiliar — nunca debe tumbar un run que ya
        # terminó su trabajo. Si la migración 00107 no está aplicada, este
        # INSERT falla y el `except Exception` de arriba marca el run como
        # `failed` DESPUÉS de haber insertado los candidatos y gastado el saldo.
        try:
